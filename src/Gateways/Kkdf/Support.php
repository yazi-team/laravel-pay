...
namespace Xiaofan\Pay\Gateways\Kkdf;
...
class Support {
    ...
    private function __construct(Config $config) {
        $this->baseUri = Kkdf::URL[$config->get('mode', Kkdf::MODE_NORMAL)];
        $this->config = $config;
    ...
    public static function requestApi(array $data, $method = 'post'): Collection {
        Events::dispatch(new Events\ApiRequesting('Kkdf', '', self::$instance->getBaseUri(), $data));

        $data = array_filter($data, function ($value) {
            return ('' == $value || is_null($value)) ? false : true;
        });
        $result = self::$instance->{$method}('', $data);
        Log::debug('Kkdf requestApi Content', [$result]);
        Events::dispatch(new Events\ApiRequested('Kkdf', '', self::$instance->getBaseUri(), $result));
    ...
    public static function generateSign(array $params): string {
        ...
        Log::debug('Kkdf Generate Sign', [$params, $sign]);
    ...
    public static function verifySign(array $data, $sync = false, $sign = null): bool {
        ...
        Log::debug('Kkdf notify Generate Sign', [$data, $sign]);
    ...
    protected static function processingApiResult($data, $result): Collection {
        if (!empty($result) && is_array($result) && $result['Code'] == 200) {
            $result['Data'] = self::rsa_decrypt($result['Data']);
            return new Collection($result);
        } else {
            Events::dispatch(new Events\SignFailed('Kkdf', '', $result));
            throw new GatewayException('Get Kkdf API Error:' . 
                    (isset($result['Message']) ? $result['Message'] : 'unkown'), 
                $result);
        }
    }
    ...
    public static function rsa_encrypt($str) {
        ...
        if (is_null($privateKey)) {
            throw new InvalidConfigException('Missing Kkdf Config -- [private_key]');
        }
    ...
    public static function rsa_decrypt($str) {
        ...
        if (is_null($publicKey)) {
            throw new InvalidConfigException('Missing Kkdf Config -- [public_key]');
        }
