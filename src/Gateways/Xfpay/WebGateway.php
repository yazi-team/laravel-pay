...
class WebGateway extends Gateway {
    ...
    public function pay($endpoint, array $payload): ResponseResult {
        ...
        Events::dispatch(new Events\PayStarted('Xfpay', 'Web/Wap', $endpoint, $payload));
        $result = Support::requestApi($payload);
        if ($result['qrcodeUrl']) {
            $response = redirect($result['qrcodeUrl']);
        }else{
            throw new GatewayException('Get Xfpay API Error:' . (isset($result['msg']) ? $result['msg'] : 'unkown'), $result);
        }
