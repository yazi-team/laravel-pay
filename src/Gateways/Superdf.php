...
use Xiaofan\Pay\Gateways\Superdf\Support;
 ...
 */
class Superdf implements GatewayApplicationInterface {
     ...
     */
    const URL = [
        self::MODE_TRANSFER => 'http://df.gogozhifu.com/Payment_Dfpay_add.html',
        self::MODE_TRANSFER_QUERY => 'http://df.gogozhifu.com/Payment_Dfpay_query.html',
    ];
    ...
    public function transfer(Transferable $transfer) {
        ...
        $this->payload['out_trade_no'] = $transfer->getTransferNo();
        $this->payload['money'] = sprintf("%.2f", intval($transfer->getAmount()) / 100);
        $this->payload['cardnumber'] = $transfer->getAccount();
        $this->payload['accountname'] = $transfer->getRealName();
        $this->payload['bankname'] = $transfer->getExtra('bank_name');
        $this->payload['subbranch'] = $transfer->getExtra('bank_branch');
        if(!$this->payload['bankname']) {
            $this->payload['bankname'] = 'bankname';
        }
        if(!$this->payload['subbranch']) {
            $this->payload['subbranch'] = 'subbranch';
        }
        $this->payload['province'] = '广东省';
        $this->payload['city'] = '深圳市';
...
//        dump($this->payload);die;
    ...
    public function verify($data = null, bool $refund = false): PurchaseResult {
        ...
        Events::dispatch(new Events\RequestReceived('Superdf', '', $data));

        \Illuminate\Support\Facades\Log::info("Superdf notify data", [$data]);
        if (Support::verifySign($data)) {
            $find = $this->find($data['out_trade_no']);
            \Illuminate\Support\Facades\Log::info("Superdf notify find data", [$find->toArray()]);
            $data['err_msg'] = isset($data['refMsg']) ? $data['refMsg'] : '';
            return new PurchaseResult('superdf', 
                    $data['out_trade_no'], 
                    $find['transaction_id'],
                    0, 
                    intval($find['refCode']) === 1, 
                    $find['success_time'], $data);
        }

        Events::dispatch(new Events\SignFailed('Superdf', '', $data));

        throw new InvalidSignException('Superdf Sign Verify FAILED', $data);
    }
    ...
    public function find($order, string $type = 'wap'): Collection {
        ...
        Events::dispatch(new Events\MethodCalled('Superdf', 'Find', $this->gateway, $this->payload));
    ...
    public function success(): Response {
        Events::dispatch(new Events\MethodCalled('Superdf', 'Success', $this->gateway));
