...
class WebGateway extends Gateway {
    ...
    public function pay($endpoint, array $payload): ResponseResult {
        ...
        Events::dispatch(new Events\PayStarted('Wppay', 'Web/Wap', $endpoint, $payload));
        
        $result = Support::requestApi($payload);
        if ($result['payUrl']) {
            $response = redirect($result['payUrl']);
        }else{
            throw new GatewayException('Get Wppay API Error:' . (isset($result['msg']) ? $result['msg'] : 'unkown'), $result);
        }
