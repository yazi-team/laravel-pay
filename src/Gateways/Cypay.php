...
use Xiaofan\Pay\Gateways\Cypay\Support;
 ...
 */
class Cypay implements GatewayApplicationInterface {
     ...
     */
    const URL = [
        self::MODE_NORMAL => 'http://api.changying-win.com/payment',
        self::MODE_DEV => 'http://api.changying-win.com/payment',
    ...
    public function __construct(Config $config) {
        ...
        $this->payload = [
            'agentNo' => $config->get('app_id'),
            'paymentType' => $config->get('pay_type'),
...
//        Events::dispatch(new Events\PayStarting('Cypay', $gateway, $params));
        $request = app('request');
        ...
        $this->payload['title'] = $charge->getBody();
...
//        dump($this->payload);die;
    ...
    public function verify($data = null, bool $refund = false): PurchaseResult {
        ...
        Events::dispatch(new Events\RequestReceived('Cypay', '', $data));

        if (Support::verifySign($data)) {
            $find = $this->find($data['orderNo']);
            return new PurchaseResult('cypay', 
                    $data['orderNo'], 
                    $data['tradeNo'], 
                    intval($find['orderAmount']), 
                    $data['status'] === "SUCCESS" && $find['orderStatus'] == 'SUCCESS', 
                    $data['payTime'], $data);
        }

        Events::dispatch(new Events\SignFailed('Cypay', '', $data));

        throw new InvalidSignException('Cypay Sign Verify FAILED', $data);
    }
    ...
    public function find($order, string $type = 'wap'): Collection {
        ...
        Events::dispatch(new Events\MethodCalled('Cypay', 'Find', $this->gateway, $this->payload));
    ...
    public function success(): Response {
        Events::dispatch(new Events\MethodCalled('Cypay', 'Success', $this->gateway));
