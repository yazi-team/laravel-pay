...
class Support {
    ...
    private function __construct(Config $config) {
        $this->baseUri = Feilong::URL[$config->get('mode', Feilong::MODE_NORMAL)];
        $this->config = $config;
    ...
    public static function requestApi(array $data): Collection {
        Events::dispatch(new Events\ApiRequesting('Feilong', '', self::$instance->getBaseUri(), $data));
        ...
        Events::dispatch(new Events\ApiRequested('Feilong', '', self::$instance->getBaseUri(), $result));
    ...
    public static function generateSign(array $params, $type = 'pay'): string {
        ...
        Log::debug('Feilong Generate Sign', [$params, $sign]);
    ...
    protected static function processingApiResult($data, $result): Collection {
        dump($result);die;
        if (isset($result['returncode']) && '00' === $result['returncode']) {
            return new Collection($result);
        }else{
            Events::dispatch(new Events\SignFailed('Feilong', '', $result));
            throw new GatewayException('Get Feilong API Error:' . $result['returncode'], $result);
        }
    }
