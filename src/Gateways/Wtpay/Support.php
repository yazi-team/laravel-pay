...
class Support {
    ...
    private function __construct(Config $config) {
        $this->baseUri = Wtpay::URL[$config->get('mode', Wtpay::MODE_NORMAL)];
        $this->config = $config;
    ...
    public static function requestApi(array $data): Collection {
        Events::dispatch(new Events\ApiRequesting('Wtpay', '', self::$instance->getBaseUri(), $data));

        $data = array_filter($data, function ($value) {
            return ('' == $value || is_null($value)) ? false : true;
        });

        $result = json_decode(self::$instance->post('', ['params' => json_encode($data)]), true);
        Events::dispatch(new Events\ApiRequested('Wtpay', '', self::$instance->getBaseUri(), $result));
    ...
    public static function generateSign(array $params, $type = 'pay'): string {
        ...
        Log::debug('Wtpay Generate Sign', [$params, $sign]);
        return $sign;
    }
    ...
    protected static function processingApiResult($data, $result): Collection {
        if (isset($result['retCode']) && 'SUCCESS' === $result['retCode']) {
            ...
            Events::dispatch(new Events\SignFailed('Wtpay', '', $result));
            throw new GatewayException('Get Wtpay API Error:' . $result['retMsg'], $result);
        }
    }
