...
namespace Xiaofan\Pay\Gateways\Ddpay;
...
use Xiaofan\Pay\Gateways\Ddpay;
use Xiaofan\Pay\Log;
...
class Support {
    ...
    private function __construct(Config $config) {
        $this->baseUri = Ddpay::URL[$config->get('mode', Ddpay::MODE_NORMAL)];
        $this->config = $config;
    ...
    public static function requestApi(array $data): Collection {
        Events::dispatch(new Events\ApiRequesting('Ddpay', '', self::$instance->getBaseUri(), $data));
        ...
        Events::dispatch(new Events\ApiRequested('Ddpay', '', self::$instance->getBaseUri(), $result));
    ...
    public static function generateSign(array $params, $type = 'pay'): string {
        ...
        Log::debug('Ddpay Generate Sign', [$params, $sign]);
    ...
    protected static function processingApiResult($data, $result): Collection {
        if (isset($result['retCode']) && 'SUCCESS' === $result['retCode']) {
            ...
            Events::dispatch(new Events\SignFailed('Ddpay', '', $result));
            throw new GatewayException('Get Ddpay API Error:' . $result['retMsg'], $result);
        }
    }
