...
class WebGateway extends Gateway {
    ...
    public function pay($endpoint, array $payload): ResponseResult {
        ...
        Events::dispatch(new Events\PayStarted('Ddpay', 'Web/Wap', $endpoint, $payload));
        $result = Support::requestApi($payload);
        if (isset($result['payParams']['payUrl'])) {
            $response = redirect($result['payParams']['payUrl']);
        }else{
            throw new GatewayException('Get Jpay API Error:' . (isset($result['msg']) ? $result['msg'] : 'unkown'), $result);
        }
