...
use Xiaofan\Pay\Gateways\Ddpay\Support;
 ...
 */
class Ddpay implements GatewayApplicationInterface {
     ...
     */
    const URL = [
        self::MODE_NORMAL => 'http://www.ddpay2.com:3020/api/pay/create_order',
        self::MODE_DEV => 'http://www.ddpay2.com:3020/api/pay/create_order',
        self::MODE_QUERY => 'http://www.ddpay2.com:3020/api/pay/query_order',
    ];
...
//        Events::dispatch(new Events\PayStarting('Ddpay', $gateway, $params));
        $request = app('request');
...
//        dump($this->payload);die;
    ...
    public function verify($data = null, bool $refund = false): PurchaseResult {
        ...
        Events::dispatch(new Events\RequestReceived('Ddpay', '', $data));

        if (Support::verifySign($data)) {
            $find = $this->find($data['mchOrderNo']);
            return new PurchaseResult('ddpay', 
                    $data['mchOrderNo'], 
                    $data['payOrderId'], 
                    intval($data['amount']), 
        ...
        Events::dispatch(new Events\SignFailed('Ddpay', '', $data));

        throw new InvalidSignException('Ddpay Sign Verify FAILED', $data);
    }
    ...
    public function find($order, string $type = 'wap'): Collection {
        ...
        Events::dispatch(new Events\MethodCalled('Ddpay', 'Find', $this->gateway, $this->payload));
    ...
    public function success(): Response {
        Events::dispatch(new Events\MethodCalled('Ddpay', 'Success', $this->gateway));
