...
class WebGateway extends Gateway {
    ...
    public function pay($endpoint, array $payload): ResponseResult {
        ...
        Events::dispatch(new Events\PayStarted('Wukong', 'Web/Wap', $endpoint, $payload));
        $result = Support::requestApi($payload);
        if ($result['url']) {
            $response = redirect($result['url']);
        }else{
            throw new GatewayException('Get Wukong API Error:' . (isset($result['msg']) ? $result['msg'] : 'unkown'), $result);
        }
