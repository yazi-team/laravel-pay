...
class Support {
    ...
    private function __construct(Config $config) {
        $this->baseUri = Yinhai::URL[$config->get('mode', Yinhai::MODE_NORMAL)];
        $this->config = $config;
    ...
    public static function requestApi(array $data): Collection {
        Events::dispatch(new Events\ApiRequesting('Yinhai', '', self::$instance->getBaseUri(), $data));
        ...
        Events::dispatch(new Events\ApiRequested('Yinhai', '', self::$instance->getBaseUri(), $result));
    ...
    public static function generateSign(array $params): string {
        ...
        Log::debug('Yinhai Generate Sign', [$params, $sign]);
    ...
    protected static function processingApiResult($data, $result): Collection {
        if (isset($result['code']) && '1' === $result['code']) {
            return new Collection($result['data']);
        } else {
            Events::dispatch(new Events\SignFailed('Yinhai', '', $result));
            throw new GatewayException('Get Yinhai API Error:' . $result['msg'], $result);
        }
    }
